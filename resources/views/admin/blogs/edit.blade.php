    ...
    <div class="animated fadeIn">
      <form id='registerForm'>
        ...
        <div class="row">
          ...
          <div class="form-group col-md-12">
            <label>Desc</label>
            <input type="text" name="desc" id="desc"  value='{{$blog->desc}}' class="form-control" placeholder="desc">
          </div>

          <div class="form-group col-md-12">
            <label>Content</label>
            <textarea name="content" id="content" class="form-control" style="height:200px ;">{{$blog->content}}</textarea>
  ...
  $('#apply').on('click', function(event) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        $.each(response.blog, function(k, v) {
        // console.log(response.user);
           $('#title').val(v.title);
           $('#content').val(v.content);
           $('#publish_date').val(v.publish_date);
           $('#desc').val(v.desc);
